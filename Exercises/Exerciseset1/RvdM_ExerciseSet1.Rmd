...
<br>
For example 'quantmod', a package for quantitative financial modelling framework. A quick google search gives many different packages that can be used for finance related activities. get
...
**Your Answers here:**
<br> *Q1* In the file `dataSet1.R` under line 8, the code says: `mytickers = c("SPY", "AAPL")`
<br> *Q2*

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
Apple = StockReturns[,"AAPL"]
round(100*sum(Apple),1)
round(100*mean(Apple),3)
round(100*sd(Apple),3)
```



*Q3* Use file `dataSet1.R` and under line 8 also include Yahoo! with ticker YHOO, so the code will look like `mytickers = c("SPY", "AAPL","YHOO"`. Subsequently repeat the steps under Q2
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
Yahoo = StockReturns[,"YHOO"]
round(100*sum(Yahoo),1)
round(100*mean(Yahoo),3)
round(100*sd(Yahoo),3)
```
<hr>
<hr>
...
<br> *Q2*
> ?apply
Repeat as under line 138-140, but now apply with 2, instead of 1, because data is transposed and you want to look at columns, not rows

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio_2 = apply(TranseposeData,2,mean)
names(portfolio_2)<- colnames(TranseposeData)
pnl_plot(portfolio_2)
```
...
**Your Answers here:**
<br> *Q1* It is allready done in the dataset (startdate in line 9), so I won't run it again

I put all the codes in this ExerciseSet, so you can rerun everything with pressing the green arrows step by step

<br> *Q2* Follow same proces, add data by finding ticker and updating dataset. Re-run all the steps as been done before
...
[1] 20
<br> *Q2* not same format (not an array/not a matrix) and 20 vs. 40 lines - Erros says non-nummer argument (data is not a number)
