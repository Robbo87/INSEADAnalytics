...
getwd()
setwd("C:/Users/Rob/Desktop/Desktop/INSEAD/P3/INSEADAnalytics/Exercises/Exerciseset2")
list.files()
```
...
Look at the variance explained table, and see that for 'cumulative pct of explained variance' to be above 50%, needs 6 components 

*Q2* 

Look at the sorted table with the weights per component, we read the futures with the heightest weight for comp1. The top futures are: 5 yr T-Notes US - 0.93, 10yr T-Notes - 0.93, Treasury Bonds - 0.86, 2yr T-Note US - 0.85, Canadian Bond - 0.79 and Eurodollar - 0.74 

Alternatively, use the following code to display the first 10 rows for Component 1:

```{r} 
as.data.frame(show_data[1:10,1,drop=F],row.names=rownames(show_data[1:10,]),col.names=colnames(show_data[,1]))
...
precrisis_data=futures_data[as.Date(rownames(futures_data))>"2007-10-01"&as.Date(rownames(futures_data))<"2009-03-01",]
PreCrisis_Variance_Explained_Table_results<-PCA(Precrisis_data, graph=FALSE)
PreCrisis_Variance_Explained_Table<-PreCrisis_Variance_Explained_Table_results$eig
colnames(PreCrisis_Variance_Explained_Table)<-c("Eigenvalue", 
  "Pct of explained variance", "Cumulative pct of explained variance")
```

```{r}
Precrisis_show_data = round(PreCrisis_Variance_Explained_Table, 2)
iprint.df(Precrisis_show_data)
```

```{r}
show_Pre_crisis_data = round(cor(futures_data),2)
iprint.df(show_Pre_crisis_data, scale=TRUE)
```

```{r echo=TRUE}
Eigenvalues_PreCrisis  <- PreCrisis_Variance_Explained_Table[, "Eigenvalue"]
```

```{r}
df           <- cbind(as.data.frame(Eigenvalues_PreCrisis  ), c(1:length(Eigenvalues_PreCrisis  )), rep(1, length(Eigenvalues_PreCrisis  )))
colnames(df) <- c("Eigenvalues_PreCrisis  ", "components", "abline")
iplot.df(melt(df, id="components"))
```
* Crisis * 
```{r echo=TRUE}
...
```{r}
crisis_show_data = round(Crisis_Variance_Explained_Table, 2)
iprint.df(crisis_show_data)
```

```{r}
show_Crisis = round(cor(futures_data),2)
iprint.df(show_Crisis, scale=TRUE)
```

```{r echo=TRUE}
Eigenvalues_Crisis  <- Crisis_Variance_Explained_Table[, "Eigenvalue"]
```

```{r}
df           <- cbind(as.data.frame(Eigenvalues_Crisis ), c(1:length(Eigenvalues_Crisis )), rep(1, length(Eigenvalues_Crisis )))
colnames(df) <- c("Eigenvalues_Crisis ", "components", "abline")
iplot.df(melt(df, id="components"))
```
...
In Crisis year, less variables are needed to predict the outcome (only 4 factors), whereas in pre-crisis years 6 factors are needed to explain over 50% of the variance.*
...
1.	The first lines first set the signal to zero and used to normalize the future tables. Next, the MAfreq variable creates a range of numbers which starts at 10 and increments by 20 until 250. Next, a positive or negative sign is created by an iterator that goes through each column and, according to future data. This depicts the predicted trajectory and the "vote sign". $1 is add to each entry in lines 193-198 . Then this is multiplied by the vote value.
2.	In order to change the strategy, new variables were created modifying the same code structure. The command line MAfreq<-seq(10,250,by=20), affects how often the trading strategy is implemented, it affects the frequency. If you use a higher frequency, the strategy becomes less sensitive, returns are reduced, and DD is increased.
